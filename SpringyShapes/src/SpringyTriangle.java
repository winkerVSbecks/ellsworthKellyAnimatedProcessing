...
public class SpringyTriangle {
	...
	SpringyTriangle(PApplet _p, PBox2D _b, PVector _topStart, PVector _topAnchor, PVector _tEnd, int _c, Vec2 _force) {
		...
		tAnchor = new Particle(p, b, topAnchor.x, topAnchor.y, true);
		tImpactor = new Particle(p, b, topMp.x, topMp.y, false);
		tStartAnchor = new Particle(p, b, topStart.x, topStart.y, true);
		tEndAnchor = new Particle(p, b, tEnd.x, tEnd.y, true);
	...
	void update() {
		...
		// move re-position the shape if the particle moves
		if(tStartAnchor.overEvent()) topStart = new PVector(tStartAnchor.x, tStartAnchor.y);
		if(tAnchor.overEvent()) topAnchor = new PVector(tAnchor.x, tAnchor.y);
		if(tEndAnchor.overEvent()) tEnd = new PVector(tEndAnchor.x, tEndAnchor.y);
		if(tImpactor.overEvent()) topMp = new PVector(tImpactor.x, tImpactor.y);
	}
